...
try {
  ...
  const configuration = await page.evaluate(() => window.noteplusCloud.configurationStatus());
  assert.equal(configuration.configured, false);
  assert.equal(configuration.firebaseVersion, "12.16.0");
  assert.equal(configuration.signInStrategy, "popup");
