...
if (!appUrl) {
  ...
  appUrl = `http://127.0.0.1:${address.port}/노트앱_v14.html?qa=${Date.now()}`;
}
...
try {
  ...
  assert.deepEqual(await page.evaluate(() => window.noteplusCloud.configurationStatus()), {configured: false, firebaseVersion: "12.16.0"});
  assert.match(await page.evaluate(() => window.noteplusCloud.friendlyError({code: "auth/popup-blocked"})), /팝업을 허용/);
  assert.match(await page.evaluate(() => window.noteplusCloud.friendlyError({code: "storage/quota-exceeded"})), /저장공간 한도/);
