...
async function verifyPersistentMobileJourney(browser) {
  ...
  await page.screenshot({ path: path.join(artifacts, "public_alpha_mobile_persist_v1.png"), fullPage: true });
...
async function verifyHonestNoStorageFallback(browser) {
  ...
  await page.screenshot({ path: path.join(artifacts, "public_alpha_mobile_storage_blocked_v1.png"), fullPage: true });
  assert.deepEqual(errors, [], errors.join("\n"));
  await context.close();
