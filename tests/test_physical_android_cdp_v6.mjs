...
const version = process.env.NOTEPLUS_VERSION || "v14";
const appUrl = new URL(`${encodeURIComponent(`노트앱_${version}.html`)}?physical=${Date.now()}`, publicUrl).href;
const expectedSha = process.env.NOTEPLUS_EXPECTED_SHA || "08961F3DCBF2651BBC9E5FA4A44F6C4AB6EE6C55B31AB21EDA3CFF22CB64768F";
...
try {
  ...
  fs.writeFileSync(path.join(artifacts, "physical_android_alpha_v14.png"), Buffer.from(screenshot.data, "base64"));
  ...
  fs.writeFileSync(path.join(artifacts,"physical_android_alpha_v14.json"),JSON.stringify(result,null,2));
