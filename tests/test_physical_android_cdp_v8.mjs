...
try {
  ...
  assert.ok(initial.manifest.endsWith(expectedManifest));
  ...
  let screenshot = {captured: false, method: "", error: "Not retained by default for privacy."};
  if (retainScreenshot) {
    // Capture only an application-generated proof card after the persisted state is restored.
    await cdp.evaluate(`(()=>{document.documentElement.innerHTML='<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;background:#f3f7f2;color:#173b23;font:600 18px system-ui;display:grid;place-items:center;min-height:100vh}main{padding:28px;border:2px solid #00a33a;border-radius:18px;background:#fff;text-align:center;max-width:300px}small{display:block;color:#52645a;margin-top:12px;font-weight:400}</style></head><body><main>NotePlusP ${version}<br>Android device QA passed<small>Privacy-safe proof · no note content captured</small></main></body>';return true;})()`);
    try {
      const captured = await cdp.send("Page.captureScreenshot", { format: "png", fromSurface: true }, 10000);
      fs.writeFileSync(path.join(artifacts, `${artifactStem}.png`), Buffer.from(captured.data, "base64"));
      ...
      try {
        ...
        fs.writeFileSync(path.join(artifacts, `${artifactStem}.png`), png);
