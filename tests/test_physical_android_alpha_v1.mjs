...
const physicalAppUrl = new URL("%EB%85%B8%ED%8A%B8%EC%95%B1_v7.html?physical=" + Date.now(), publicUrl).href;
...
async function verifyRelease() {
  const response = await fetch(new URL("%EB%85%B8%ED%8A%B8%EC%95%B1_v7.html?physical=" + Date.now(), publicUrl), { cache: "no-store" });
...
try {
  ...
  await page.screenshot({ path: path.join(artifacts, "physical_android_alpha_v1.png"), fullPage: true });
