...
## 2026-07-23 · 공개 v16 핵심 여정·접근성 회귀 통과 (추가)

- ENEX 취소·재시도·첨부 중복 보호, JSON 내보내기·복원·재접속, 360/390px 화면, 키보드·ARIA, 알파 핵심 저장 여정을 현행 공개 v16 SHA에서 통과했다.
- 격리 브라우저 프로필만 사용했고 각 테스트의 추가 데이터는 원상복구 서명으로 제거됐다.
- favicon 404는 앱 여정을 막지 않는 P2로만 남는다. 실제 Google 전송·PC 실데이터 감사·파일럿을 통과로 기록하지 않는다.

## 2026-07-23 · 공개 v16 ENEX 완전성·대규모 manifest 회귀 통과 (추가)
...
## 2026-07-23 · 공개 v16 JSON 독립 스키마 검사 통과 (추가)
