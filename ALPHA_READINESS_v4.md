...
이 문서는 v7 실기기 검증 이후의 제한 알파 상태 정본이다. `ALPHA_READINESS.md`, `ALPHA_READINESS_v2.md`, `ALPHA_READINESS_v3.md`, `LIMITED_ALPHA_PLAN_v1.md`는 이전 시점의 역사적 기준으로 보존한다.
...
- 사실: Samsung SM-G996N(Android 15, Chrome 150) 실제 기기에서 411×781 CSS viewport, IDB 저장·재접속, 합성 ENEX PDF Blob 복원, 서비스워커 오프라인 재진입, 모바일 설치 버튼 접근 경로를 통과했다.
- 사실: 실기기 검사는 시작 상태를 서명으로 보존한 뒤 QA 노트와 첨부를 제거했으며 종료 시 QA 0건·ENEX 0건·상태 서명 일치를 확인했다.
...
- 외부 의존성: 개인정보를 안전하게 처리한 실제 한국어 ENEX 1세트, 비개발자 파일럿 3~5명. iPhone 교차 확인은 남은 플랫폼 위험이다.
...
| 2. 실제 모바일 핵심 여정 | 통과 | 실제 Samsung Android에서 생성·수정·재접속, ENEX PDF, 오프라인 재진입, 원상복구 통과 |
...
`공개 v7·실제 Android 핵심 여정` → `실제 한국어 ENEX 1계정 이전` → `파일럿 3~5명` → `P0 즉시 수정·재현된 P1 최소 수정` → `종료 보고`
...
6. [사용자] 개인정보를 정리한 실제 한국어 ENEX 1세트 제공 또는 감독 세션
7. [공동] 실제 ENEX 노트·첨부·노트북 수량과 실패 대조
...
5. [완료] 실제 Android IDB·ENEX PDF·오프라인·설치 경로·원상복구
6. [다음] 파일별 노트·첨부·실패 합계 대시보드
7. 수량·파일·첨부 기반 최종 완료/확인 필요 판정
8. 가져오기 취소·재시도·중복 무변경 회귀
9. JSON 백업→초기화→복원 합성 훈련
10. 이미지·PDF·오디오 저장·재접속 회귀
11. IndexedDB만 차단/localStorage 보조 저장 회귀
...
- `tests/test_physical_android_alpha_v1.mjs`
...
- `artifacts/physical_android_alpha_v1.png`
- `artifacts/physical_android_alpha_v1.json`

## 7. 다음 실행

사용자 승인 없이 가능한 최상위 작업은 파일별 노트·첨부·실패 합계 대시보드다. 실제 한국어 ENEX가 준비되면 외부 의존 임계 경로를 즉시 우선한다.
