...
import {auditReleaseEvidence} from './audit_release_evidence_v22.mjs';
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..');
export function evaluateGate(owner,evidence,readEvidence=()=>false,now=Date.now(),auditResult={auditPassed:true}){
 const blockers=[];
 ...
 if(auditResult?.auditPassed!==true)blockers.push('EVIDENCE:audit');
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)){
 ...
 const audit=auditReleaseEvidence(evidence);
 const result=Object.assign(evaluateGate(owner,evidence,relative=>{if(typeof relative!=='string'||path.isAbsolute(relative))return false;const target=path.resolve(root,relative);return target.startsWith(root+path.sep)&&fs.existsSync(target)&&fs.statSync(target).isFile()&&fs.statSync(target).size>0;},Date.now(),audit),{audit});
