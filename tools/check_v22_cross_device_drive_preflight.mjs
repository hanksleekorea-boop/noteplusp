...
export function evaluatePreflight({clientId,pcUrl,androidUrl,adbOutput='',cloudProjectNumbers=null}){
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)){
 const result=evaluatePreflight({clientId:readClientId(),pcUrl:process.env.NOTEPLUS_PC_V22_URL||'',androidUrl:process.env.NOTEPLUS_ANDROID_V22_URL||'',adbOutput:commandText('adb',['devices','-l']),cloudProjectNumbers:cloudProjectNumbers()});
